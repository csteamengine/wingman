            ...
            {aiPresets.map((preset: AIPreset) => (
                ...
                >
                    <div className="flex-1">
                        <p className="text-sm font-medium text-[var(--editor-text)]">{preset.name}</p>
                        <p className="text-xs text-[var(--editor-muted)] mt-0.5">{preset.description}</p>
                    </div>
                    <button
                        onClick={() => togglePresetEnabled(preset.id)}
                        className={`relative w-11 h-6 rounded-full transition-colors ml-3 ${
                            preset.enabled ? 'bg-[var(--editor-accent)]' : 'bg-[var(--editor-border)]'
                        }`}
                    >
                        <span
