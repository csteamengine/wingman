                ...
                {activeTab === 'settings' && (
                    <div className="space-y-5">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center justify-between py-2">
                                <div className="flex-1">
                                    <label className="block text-sm font-medium text-[var(--ui-text)]">Show Tips</label>
                                    <p className="text-xs text-[var(--ui-text-muted)] mt-0.5">Display helpful tips at the bottom of the editor</p>
                                </div>
                                <button
                                    onClick={() => handleUpdate({show_tips: !settings.show_tips})}
                                    className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 ${
                                        settings.show_tips ? 'bg-[var(--ui-accent)]' : 'bg-[var(--ui-border)]'
                                    }`}
                                >
                                    <span
                                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
                                            settings.show_tips ? 'translate-x-5' : 'translate-x-0'
                                        }`}
                                    />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-between py-2">
                                <div className="flex-1">
                                    <label className="text-sm font-medium text-[var(--ui-text)]">
                                        Colorblind Mode (Diff View)
                                    </label>
                                    <p className="text-xs text-[var(--ui-text-muted)] mt-0.5">
                                        Use blue/orange colors instead of green/red in diff previews
                                    </p>
                                </div>
                                <button
                                    onClick={() => handleUpdate({colorblind_mode: !settings.colorblind_mode})}
                                    className={`relative w-11 h-6 rounded-full transition-colors flex-shrink-0 overflow-hidden ${
                                        settings.colorblind_mode ? 'bg-[var(--ui-accent)]' : 'bg-[var(--ui-border)]'
                                    }`}
                                >
                                    <span
                                        className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
                                            settings.colorblind_mode ? 'translate-x-5' : 'translate-x-0'
                                        }`}
                                    />
                                </button>
                            </div>
